...
from app.data.mcp_tool_fallback import BOOTSTRAP_TOOLS, MCP_TOOL_TO_META
...
def validate_playbook(playbook: SkillPlaybook) -> list[str]:
    ...
    for tool in bootstrap:
        ...
        if tool not in BOOTSTRAP_TOOLS:
            logger.warning(
                "playbook bootstrap tool '%s' differs from mcp_tool_fallback.BOOTSTRAP_TOOLS",
                tool,
            )

    reports = playbook.phase("mcp_reports")
    if reports and reports.tools_from_meta:
        for tool in MCP_TOOL_TO_META:
