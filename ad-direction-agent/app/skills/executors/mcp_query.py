...
class McpQuerySkillExecutor:
    ...
    async def run(
        ...
        planned: list[str] = list(bootstrap_phase.tools if bootstrap_phase else _BOOTSTRAP_TOOLS)
        ...
        adapter = McpAdapter()
        bootstrap_set = set(self.playbook.bootstrap_tools or _BOOTSTRAP_TOOLS)
        bootstrap_timeout = (
