...
"""【codex/8012 版】每日批量任务：拉测试 ASIN → 调 8012 /campaign/viewmodel → 写 3308 测试 ERP.

这是 chenv31 `batch_via_api.py` 的 codex 副本，只改目标与数据源，逻辑一致：
- 调 **8012**(preview_boot，USE_TEST_LISTING=1) 的 /campaign/viewmodel（含 codex 复核）
- ASIN 来源：**3308 测试 ERP** t_advert_agent_decision_config（连接从 vendor settings 动态读，不硬编码）
- 写入落 **3308 测试库**（隔离，不碰 prod ERP / 8010）
- asyncio + httpx 并发 5；每 ASIN 重试 1 次（30s 延迟）；超时 600s/ASIN；实时落状态文件

用法:
    python3.11 batch_via_api_codex.py                     # 跑 3308 里全部测试 ASIN
...
# vendor 上 path，复用 8012 的 settings 取 3308 测试 ERP 连接（不硬编码密码）
...
def fetch_asin_list_from_test_erp() -> list[str]:
    """从 3308 测试 ERP t_advert_agent_decision_config 拉 ASIN 列表（同 prod 逻辑，仅换库）。"""
    if ERP_CONN is None:
        raise RuntimeError(
...
async def amain() -> int:
    ...
    if args.asins:
        ...
        log.info("从 3308 测试 ERP t_advert_agent_decision_config 拉 ASIN ...")
        asins = fetch_asin_list_from_test_erp()
