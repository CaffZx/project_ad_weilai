...
> **最后更新**: 2026-08-07（版本日志见文末，最新 v3.28：组合预算瓶颈 P12 + 否词知识库诊断）
> **版本**: v3.28
> **分支**: chenv3.2
...
| 08-07 | **组合预算瓶颈 P12 + 自动活动出价 + current_portfolio 重命名 + 否词知识库诊断** | `campaign_guardrails.py`：新增 P12_PORTFOLIO_BOTTLENECK——组合 7 天利用率 ≥100% 且样本不足→禁淘汰/禁调 bid 预算，`action=keep` + `triggered_rule=PORTFOLIO_BOTTLENECK`（KB23 §8.4）。`reasoner.py`：广泛流 prompt 注入逐活动组合利用率 + 瓶颈感知。`campaign.py`：`_pf_util` 计算+注入流分析/护栏；`broad_list` 改为 BROAD/PHRASE（排除 AUTO）+ Redis `_last_shop_account` 回写修复。`campaign_fetcher.py`：自动活动识别 + `_fetch_auto_targets` 批量拉四种匹配类型出价。`ai_portfolio_class` → `current_portfolio` 全量重命名（models/budget_realloc/mappers/frontend 22 文件）。新增 `migrate_card_current_portfolio.sql`。新增 否词知识库原文引用 + 否词规则矛盾诊断与完善方案（KB31 §4/KB19 §8/KB17 §3/ONT-RUNTIME-004 全面审计，发现否词触发仅由语义判定驱动、无数据驱动规则、R1/R2 词永不否词等系统性缺陷）。`reasoner.py`：预算回算 prompt 新增 `group_budget_floor` 约束（组合预算不得低于组内最大活动预算）。`campaign_budget_reallocation.py`：`group_budget_floor` 逐组计算（挪组后组内活动预算最大值）。`campaign.py`：`_reconcile_portfolio_targets` current_portfolio 兜底 `unit.current_portfolio_name`。前端 render.js：挪组行展示（当前组→目标组）。`campaign_viewmodel.py`：`current_portfolio_class` 字段（card.current_portfolio） |
...
*v3.28: 组合预算瓶颈P12护栏 + 自动活动出价 + 新增流颜色词/节日词 + current_portfolio重命名（2026-08-06~07，本地未部署服务器）—— ①新增 P12_PORTFOLIO_BOTTLENECK：组合利用率≥100%且样本不足→禁淘汰/禁调bid预算，force keep ②广泛流 prompt 注入组合利用率+PORTFOLIO_BOTTLENECK感知 ③broad_list 改为BROAD/PHRASE（排除AUTO）④campaign_fetcher 自动活动识别+_fetch_auto_targets批量拉四种匹配类型出价 ⑤新增流颜色词/节日词标记+Listing五点注入（erp_listing_product_info）⑥paused proposed_budget=0.0 ⑦_derive_match_type 全来源→BROAD ⑧ai_portfolio_class→current_portfolio全量重命名(22文件)+前端挪组行(当前组→目标组) ⑨Redis _last_shop_account回写修复 ⑩group_budget_floor逐组计算(组合预算≥组内最大活动) ⑪否词知识库原文引用+矛盾诊断报告。*
