...
2. Campaign 分析链路：消费 Campaign 粒度数据，生成活动级调整建议（含淘汰/暂停）、预算组合、新建活动、复评建议；广泛/词组/自动流的搜索词提精准候选作为搜索词来源，与流量来源在新建活动公共链路合流。
...
- 搜索词提精准不是独立的第三条 MCP 管道：活动样本门禁通过后由广泛/词组/自动流同批搜索词数据产出，经过 bundle 过滤和确定性准入后，与流量来源按词去重合流。
...
- Campaign 搜索词窗口、样本门禁或新增活动来源变化时，同步更新 `08`、`03`、`04`、`01` 和 `14`。
