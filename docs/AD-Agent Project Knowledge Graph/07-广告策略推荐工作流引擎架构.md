...
时序：`_backfill_campaign_adjustment_context()` 在护栏执行之前调用，保证 `item.is_core` 在 P0/P3/P5 消费前已设置。R2/R3/R4 重判产生的新 item 会被二次 backfill 覆盖，`is_core` 不丢失。
...
| Config | `POST /long-term-config/{asin}/save-all` | 写+回读 | **一键保存(2026-08-04)**：策略/P3/方向 5 字段 + 战略 3 列(从 state DB 补齐)一次 atomic upsert 到 `t_advert_agent_config`，state DB 三处幂等镜像，返回最新 snapshot；`SaveAllConfigRequest` 校验 min_length=1/ge/le/gt，战略不完整 409 |
| Config | `GET /long-term-config/{asin}/latest` | 读 | C 态可编辑控件回读 Agent 配置表真源(8 列反向映射为中文枚举)，空行返回全空 snapshot(为空显空)；按 `(parent_asin,parent_seller_sku,shop_id)` 查 |
