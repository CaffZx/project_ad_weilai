...
- KB 注入：`ad-direction-agent/app/llm/kb_loader.py`（`semantic_core` preset → KB29 §1-6）
...
| 3 | `ad_campaign_product_keyword_list` | StarRocks | campaign_name, keyword_text, match_type, child_asin | 关键词发现，决定 Step4/5 循环范围 |
| 4 | `ad_campaign_product_report × N` | StarRocks | cost, orders, acos（14 天窗口） | data_core：高广告花费 + 广告效果已验证 |
...
Step1 串行，Step2-3 并行，Step4-6 并行。Azlisting 工具由 `CoreKeywordFetcher` 内部持独立 `StreamableHttpMcpInvoker` 调用，不走 `McpAdapter` 全局路由。
...
多关键词活动标记 `_multi_kw`，其下所有关键词跳过 data_core 四个条件，仅靠 semantic_core 判断。
...
LLM 使用 `temperature=0.3`、JSON 模式、不需要 thinking，注入 KB29 §1-6。
...
Campaign 引擎在 `_analyze_campaigns_impl()` 中通过 `fetch_core_keyword_set()` 读取最近一次 DONE 批次的核心词集合，分别注入 LLM prompt（通过 `_analyze_one_stream()` 的 `is_core` 字段）和 backfill 到 `CampaignAdjustmentItem.is_core`。下游护栏 P0/P3/P5、ERP card 写入 `is_core` 列、前端 viewmodel 均自动贯通。
...
| `core_keyword_analyze_enabled` | `False` | 离线 endpoint 是否允许执行，需显式开启 |
...
离线独立 endpoint `POST /core-keyword/analyze`，不串入 `/campaign/viewmodel` 主链路。批跑从 `t_advert_agent_decision_config` 取三元组列表，7 天一次 cron 调度。主 campaign 流程只读已完成标签，不现场补算。
