...
新建活动展示：后端 `new_campaigns` 经 ERP snapshot 生成 CREATE card；`viewmodel.js` 将其归一化为 `item_type="new"`、`action="create_campaign"`，render 层按“新增活动”卡展示。来源 A（候选词流）与来源 B（广泛/词组/自动搜索词提精准）在后端合流后共用这条展示和确认链路，前端不区分来源，也不直接拼接 pending。
