...
| 一键保存入口 | `ad-asisitant-agent.html` `saveAllConfig()` | 收集 5 字段 → `POST /save-all` → 回读 + 副作用 |
| 配置回读 | `applyConfigSnapshotToEditable(snap)` | 回填 8 字段到所有可编辑控件,空显空 |
| 保存栏门禁 | `_syncSaveAllBar()` | C 态 `in_progress` 真值判定 |
| 面板加载统一 | `_ensureInsightPanelsLoaded()` | 共享 promise 缓存 + `diagnosisLoaded` 短路 |
...
- 回读对称:`loadAll` 在 `loadStrategy/loadTactics` 完成后立即调 `GET /long-term-config/{asin}/latest`(**不等待诊断/P3/方向 LLM 链**)→ `applyConfigSnapshotToEditable`,空行显空;与保存后回读同一套函数。
- **回读值独占(2026-08-05)**:灰值预览已切除——`autoFillLeftInputs`/`_setP3SavedFlags` 已删除,P3 段(`loadP3Rec`/`refreshP3`)不再写 ACOS/预算输入框与脏基准;`enableTabsForReturnVisit` 面板加载改后台(`_ensureInsightPanelsLoaded().then`);`loadExecution` 替换方向选项后按 `_latestConfigSnap.directions` 重勾(弃 state `selected_directions`)。进入即回读黑值,后续加载不覆盖。
- 面板加载统一:`_ensureInsightPanelsLoaded()` — `diagnosisLoaded` 短路 + `_insightPanelsLoadPromise` 共享缓存 + `.finally` 清理;`enableTabsForReturnVisit` 与一键保存路径均 `.then/.catch` 后台加载,不阻塞回读与保存结果。防并发双拉。
...
新建活动展示：后端 `new_campaigns` 经 ERP snapshot 生成 CREATE card；`viewmodel.js` 将其归一化为 `item_type="new"`、`action="create_campaign"`，render 层按“新增活动”卡展示。流量来源（候选词流）与搜索词来源（广泛/词组/自动搜索词提精准）在后端合流后共用这条展示和确认链路，前端不区分来源，也不直接拼接 pending。
...
- **paused `proposed_budget=0.0` 的 falsy 陷阱**：Python `or` 和 JS `\|\|` 都会把 `0.0` 判为 falsy 回退到 `current_budget`，导致"调整后预算"未减。正确写法是判 `!= null` 或显式 `action=='paused' ? 0 : ...`(render.js 执行确认弹窗已修,mappers.py:125 和 render.js 预算求和栏 L628 待修)。
- **一键保存 409 前置依赖**：`save-all` 端点从 state DB `long_term_config` 读战略 3 列补齐 8 列 upsert，若战略层未保存或 state 缺列则 409。C 态门禁不校验此依赖，前端判断一键保存按钮可见 ≠ 战略层已保存。
