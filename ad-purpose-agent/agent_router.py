    ...
    if _keywords:
        for kw in _keywords:
            ...
            if rank is None:
                ...
                chg = kw.get("rank_change_14d", 0) or 0
                if chg >= 5:
                    trend_label = "快速上升"
                elif chg >= 1:
                    trend_label = "缓慢上升"
...
- 产品阶段: {stage}
- 淡旺季: {season}
- Core keyword avg natural rank: {(metrics.get('avg_nature_rank') or 100):.1f}
- Rating: {(metrics.get('avg_star') or 4.0):.1f}
- Price: ${avg_price:.2f} (Competitor reference: ${_comp_price:.2f})
- Net CVR: {((metrics.get('net_cvr') or 0) * 100):.2f}%
- CTR: {((metrics.get('ctr') or 0) * 100):.2f}%
- Last {days}d natural order ratio: {((metrics.get('natural_order_ratio') or 0) * 100):.2f}% (Total orders: {orders_total}, Ad orders: {metrics.get('ad_orders', 0)})
- ACOS: {((metrics.get('avg_acos') or 0) * 100):.2f}%
- CPC: ${cpc:.2f} | Unit gross profit: ${gross_profit:.2f} {cpc_warning}
- Refund rate: {refund_rate_pct:.1f}%
- Est. sellable days: {_stock_days} days
...
2. `target_scores` array: ALL 5 targets with their scores, even if less than 50. Format example: [{{"target": "Traffic", "score": 45, "reason": "stage not ideal"}}]. Sort by score descending.
3. For each keyword in the core keywords list, determine its strategy type and put into `keyword_analysis` array. The `action` MUST be in Chinese and differentiate based on the 14d trend AND whether the keyword has a natural ranking:
...
4. Also reflect each keyword's strategy type and trend-aware suggestion in the `html_report`.
5. Based on the product's current stage and keyword strategy types, give concrete ad operation instructions.
6. NEVER violate the 6 [AI Absolute Red Lines] in the knowledge base!
7. Output `summary` (1-2 sentences of key observations) and `final_action` (bullet-point action items).
